"""
runs phase two unsupervised adaptation from a pretrained source
domain checkpoint, saving optimizer state alongside model weights
so that a run can be resumed from any saved checkpoint
"""
import argparse
import os
...
def main():
    ...
    if args.resume_from is not None:
        resume_ckpt = torch.load(args.resume_from, map_location=config.device)
        model.load_state_dict(resume_ckpt["model"])
        rad_reg.load_state_dict(resume_ckpt["rad"])
        discriminator.load_state_dict(resume_ckpt["disc"])
        adapt_optimizer.load_state_dict(resume_ckpt["optim_model"])
        disc_optimizer.load_state_dict(resume_ckpt["optim_disc"])
    ...
    baseline_lgg = validate_on_loader(model, lgg_val_loader, config.device, "lgg baseline")
    print("lgg baseline before adaptation:", baseline_lgg)

    for epoch in range(start_epoch, config.adapt_epochs + 1):
            ...
            torch.save(
                {
                    ...
                    "optim_model": adapt_optimizer.state_dict(),
                    "optim_disc": disc_optimizer.state_dict(),
                },
